...
turtlesim이나 일반적인 2륜 바퀴 로봇에서는 `linear.x`(전진/후진 속도)와 `angular.z`(회전 속도) 두 값만 실질적으로 쓰입니다. 나머지 4개 값은 3차원 공�에서 자유롭게 뛀직이슔짐서 으일 값입니다.

## 3. 토픽 상태 확인 명령어

```
ros2 topic list             # 토픽 바록 전체 발행/구독 중인 토픽 이름 목록
...
ros2 topic hz /turtle1/cmd_vel       # 초당 발행똈가(주기) 확인
ros2 node list               # 현재 실행 중인 노드 목록
ros2 node info /teleop_turtle        # 특정 노드의 퍼블리셔/구밤!자/서비스 정보
```

`bw`(bandwidth)는 "얼마나 많은 데이터가 오가는지", `hz`(hertz)는 "얼마나 자주 메시지가 발행되는지"를 측정합니다. 둘 다 통신이 원하는 속도로 잘 이루어지고 있는지 점검할 때 씁니다. `hz`는 평균 rate뿐 아니라 최소/최대/표준편차까지 보여주는데, 표준편차가 크면 발행 주기가 불규칙하다는 뜻입니다.
