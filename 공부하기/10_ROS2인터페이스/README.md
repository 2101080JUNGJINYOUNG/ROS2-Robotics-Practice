...
예를 들어 `geometry_msgs/msg/Twist`는 `.msg` 파일로 정의된 인터페이스이고, 그 안에 `linear`, `angular` 필드가 있다고 미리 약속되어 있는 것입니다. 인터페이스는 "설계도" 역할만 하고, 데이터가 실제로 어떻게 전달되는지는 통신 방식(토픽/서비스/액션)이 결정합니다.
...
- "센서 값이 몇 초마다 계속 필요한 것"은 토픽, "한 번 물어보고 답만 받으면 되는 것"(지도 저장, 파라미터 조회 등)은 서비스가 적합합니다.
