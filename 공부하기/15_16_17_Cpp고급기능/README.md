...
```cpp
int base = 10;
auto addBase = [base](int x) { return x + base; };  // base를 값으로 캡처
auto addBaseRef = [&base](int x) { return x + base; }; // base를 참조로 캡처
```
...
calculator = [](int a, int b) { return a + b; };   // 덧셈 람다
calculator = [](int a, int b) { return a - b; };   // 뺄셈 람다로 교체
```

함수 포인터도 비슷하지만 캡처가 있는 람다(클로저)를 담을 수 없다는 한계가 있습니다. `std::function`은 캡처 유무와 관계없이 전부 담을 수 있어 더 안전하고 유연합니다. rclcpp의 콜백 등록(`create_subscription`, `create_wall_timer`)은 내부적으로 이 `std::function`을 매개변수 타입으로 받습니다.
...
int count = std::count_if(vec.begin(), vec.end(), isGreaterThan50); // 50보다 큰 값의 개수
```
...
auto timer = node->create_wall_timer(100ms, callback);  // 100밀리초마다 callback 호출
...
**문제 상황(raw 포인터)**: `new`로 할당한 메모리는 반드시 `delete`로 해제해야 하는데, 코드가 복잡해지면 깜빡하기 쉽고(메모리 누수), 이미 해제된 메모리를 또 참조하는 실수도 생깁니다.

**`unique_ptr`**: 하나의 소유자만 가질 수 있는 스마트 포인터. 스코프를 벗어나면 자동으로 `delete`가 호출됩니다. 소유권을 넘기려면 `std::move`를 씁니다(복사는 금지 — 소유자가 둘이 될 수 없음).

```cpp
std::unique_ptr<CTest> ptr1 = std::make_unique<CTest>();
std::unique_ptr<CTest> ptr2 = std::move(ptr1);  // 소유권 이동, ptr1은 비어있음
```

**`shared_ptr`**: 여러 곳에서 동시에 소유할 수 있는 스마트 포인터로, 내부적으로 참조 카운트(reference count)를 유지합니다. 카운트가 0이 되는 순간 객체의 소멸자가 호출되고 메모리가 해제됩니다.

```cpp
auto p1 = std::make_shared<CTest>();   // 참조 카운트 1
{
    auto p2 = p1;                       // 참조 카운트 2
}                                        // p2 소멸 -> 참조 카운트 1
// p1도 소멸하면 참조 카운트 0 -> 소멸자 호출
```
